...
export const sendMessage = async (req, res) => {
  try {
    ...
    // Summary generation happens after the response. Handle failures here so
    // they cannot reach the controller's catch block after headers are sent.
    updateSummaryIfNeeded(chat._id).catch((summaryError) => {
      console.error("Failed to update chat summary:", summaryError);
    });
  } catch (err) {
    console.log(err);
    res.status(500).json({
