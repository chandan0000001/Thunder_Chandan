...
const server = http.createServer((req,res)=>{
    // here we need to handle the three cases
    // 1.verify number or > 0 -> invalid url 
    //2. greater than 100 we cant fetch
    // 3.we need to show the 100 profile names 
    const limitx = Number(req.url.slice(1));
  
    if(isNaN(limitx) || limitx <= 0){
         return res.end("Invalid Url");
    }else if( limitx > 100){
        return res.end("We cant fetch more than 100 users");
    }else if( limitx < 100){
        const arr = [];
        for(let i =0 ;i<Number(limitx);i++){
            arr.push(gitHub[i]);
            
        }
        return res.end(JSON.stringify(arr));
    }else{
        res.end("BAS KARR BHAII");
    }
})

server.listen(3646,()=>{
    console.log("Zinda he server at PORT 3646");
})